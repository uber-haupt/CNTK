...
public:
    ...
    ///
    explicit NDShape(size_t numAxes, size_t dimension = InferredDimension) : m_shapeDims(numAxes, dimension) {}

    ///
    /// Construct a NDShape instance with specified dimensions.
    ///
    NDShape(const std::vector<size_t>& dimensions) : m_shapeDims(dimensions) {}

    ///
    /// Construct a NDShape instance with specified dimensions.
    ///
    NDShape(const std::initializer_list<size_t>& dimensions) : m_shapeDims(dimensions) {}
    ...
    NDShape SubShape(size_t beginAxisId = 0, size_t endAxisId = SIZE_MAX) const
    {
        ...
        if ((endAxisId < beginAxisId) || (endAxisId > Rank()))
            InvalidArgument("NDShape::SubShape: The specified endAxisId (%zu) must not exceed the rank (%zu) of 'this' "
                            "NDShape and must be >= than the specified beginAxisId (%zu)",
                            endAxisId, Rank(), beginAxisId);
    ...
    ///
    /// Returns a boolean value indicating if the dimension size for any of the axes of 'this' shape is unknown/inferred
    /// (aka == NDShape::InferredDimension).
    ///
    ...
    ///
    /// Returns a boolean value indicating if the dimension size for any of the axes of 'this' shape is free (aka ==
    /// NDShape::FreeDimension).
    ///
    ...
    size_t TotalSize() const
    {
        if (HasUnboundDimension())
            RuntimeError("NDShape::TotalSize: TotalSize cannot be determined for a NDShape '%S' with one or more "
                         "dimensions being InferredDimension or FreeDimension.",
                         AsString().c_str());
    ...
    ///
    /// Creates and returns a new shape constructed by appending the dimensions of the specified 'shape' to 'this'
    /// shape's dimensions.
    ///
...
static const unsigned long SentinelValueForAutoSelectRandomSeed =
    std::numeric_limits<unsigned long>::max() - 2; // An arbitrary choice of sentinel value
...
struct StreamInformation
{
    StreamInformation()
        : m_id(0),
          m_storageFormat(StorageFormat::Dense),
          m_elementType(DataType::Unknown),
...
inline bool operator==(const StreamInformation& left, const StreamInformation& right)
{
    return ((left.m_id == right.m_id) && (left.m_name == right.m_name) &&
            (left.m_storageFormat == right.m_storageFormat) && (left.m_elementType == right.m_elementType) &&
            (left.m_sampleLayout == right.m_sampleLayout));
}

// Some projects require only some generic data types/interfaces from this file, and do not want to link explicitly to
// CNTKv2Library. In this case they have to define CNTK_HEADERONLY_DEFINITIONS before including CNTKLibrary.h
...
public:
    ...
    /// where a device needs to be specified and where none was explicitly provided. If no default device has been
    /// specified with a call to TrySetDefaultDevice, on the first invocation, this methods will auto-select one of the
    /// available (non-locked) devices as the default. Returns a descriptor of the globally default device.
    ///
    ...
    /// (cooperative) lock on the device (GPU). The default device can only be changed if it has not yet been frozen by
    ...
    /// a device lock. This locking mechanism allows CNTK processes to avoid device oversubscription only if they
    /// collectively choose so. In other words, the device locked by one CNTK process, can still be accessed by another
    /// CNTK process without acquiring any locks (i.e, the existing device lock can be ignored by other CNTK processes).
    /// This cooperative locking mechanism does not guarantee any kind of exclusive access to the device. The proper way
    /// to ensure exclusivity is to use tools provided by NVIDIA (nvidia smi).
    ///
    ...
    ///
    /// Static method to specify a list of excluded devices that cannot be used as globally default (neither
    /// auto-selected nor explicitly specified by 'TrySetDefaultDevice'). For example, to avoid auto-selecting the CPU,
    /// invoke 'SetExcludedDevices({DeviceDescriptor::CPUDevice()})'. However, after the default device has been
    /// selected and frozen (by a call to UseDefaultDevice()), invoking this methods has no effect, it becomes
    /// essentially a no-op.
    ///
...
private:
    DeviceDescriptor(unsigned int deviceId, DeviceKind deviceType) : m_deviceId(deviceId), m_deviceType(deviceType) {}
...
/// The actual underlying storage is either external or internal in which case its lifetime is managed through reference
/// counting.
///
...
public:
    ...
    ///
    CNTK_API NDArrayView(::CNTK::DataType dataType, const NDShape& viewShape, void* dataBuffer,
                         size_t bufferSizeInBytes, const DeviceDescriptor& device, bool readOnly = false);
    ...
    /// as large as the total size of the specified 'viewShape' and must outlive the created NDArrayView object.
    ///
    NDArrayView(::CNTK::DataType dataType, const NDShape& viewShape, const void* dataBuffer, size_t bufferSizeInBytes,
    ...
    ///
    /// Construct a NDArrayView with newly allocated sparse storage in SparseCSC format on the specified 'device' and
    /// initialize its contents with the specified Sparse CSC format data.
    ///
    CNTK_API NDArrayView(::CNTK::DataType dataType, const NDShape& viewShape, const SparseIndexType* colStarts,
                         const SparseIndexType* rowIndices, const void* nonZeroValues, size_t numNonZeroValues,
    ...
    ///
    CNTK_API NDArrayView(::CNTK::DataType dataType, ::CNTK::StorageFormat storageType, const NDShape& viewShape,
                         const DeviceDescriptor& device);
    ...
    ///
    template <typename ElementType>
    NDArrayView(const NDShape& viewShape, ElementType* dataBuffer, size_t numBufferElements,
                const DeviceDescriptor& device, bool readOnly = false)
    ...
    ///
    template <typename ElementType>
    NDArrayView(const NDShape& viewShape, const ElementType* dataBuffer, size_t numBufferElements,
                const DeviceDescriptor& device)
        : NDArrayView(AsDataType<ElementType>(), viewShape, dataBuffer, numBufferElements * sizeof(ElementType), device)
    {
    }
    ...
    ///
    /// Construct a NDArrayView with the buffer underlying the specified std::vector or std::array being the underlying
    /// storage. The container must be at least as large as the total size of the specified 'viewShape' and should
    /// outlive the created NDArrayView object.
    ///
    template <typename ContainerType,
              typename std::enable_if<
                  std::is_same<ContainerType, std::vector<typename ContainerType::value_type>>::value ||
                  std::is_same<ContainerType,
                               std::array<typename ContainerType::value_type,
                                          sizeof(ContainerType) / sizeof(typename ContainerType::value_type)>>::value>::
                  type* = nullptr>
    ...
    ///
    /// Construct a read-only NDArrayView with the buffer underlying the specified std::vector or std::array being the
    /// underlying storage. The container must be the same size as the total size of the specified 'viewShape' and
    /// should outlive the created NDArrayView object.
    ///
    template <typename ContainerType,
              typename std::enable_if<
                  std::is_same<ContainerType, std::vector<typename ContainerType::value_type>>::value ||
        ...
        : NDArrayView(viewShape, sourceContainer.data(), sourceContainer.size(), DeviceDescriptor::CPUDevice())
    {
        if (sourceContainer.size() != viewShape.TotalSize())
            InvalidArgument(
                "The size (%zu) of the STL container does not match the size (%zu) of the specified viewShape '%S'.",
    ...
    ///
    template <typename ElementType>
    explicit NDArrayView(const ElementType& value, const NDShape& viewShape = {1},
                         const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(), bool readOnly = false)
    ...
    ///
    /// Construct a NDArrayView over newly allocated dense storage on the specified device and assign the specified
    /// value to each element of the view. The specified value is cast to the specified DataType.
    ///
    explicit NDArrayView(double value, DataType dataType = DataType::Float, const NDShape& viewShape = {1},
                         const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(), bool readOnly = false)
    ...
    ///
    template <typename ElementType>
    CNTK_API std::tuple<const ElementType*, const SparseIndexType*, const SparseIndexType*, size_t>
    SparseCSCDataBuffers() const;

    ///
    /// Returns a read-only pointer to the data buffer in sparse block column format underlying 'this' view
    ///
    template <typename ElementType>
    CNTK_API std::tuple<const void*, const SparseIndexType*, const SparseIndexType*, size_t, size_t, size_t>
    SparseBlockColumnDataBuffers() const;
    ...
    /// if old col2BlockId[i] contains value at that column, it would be copied over; otherwise the block would be
    /// filled with zeros
    ///
    ...
    ///
    /// Fill 'this' NDArrayView with the specified value. The underlying DataType of 'this' view should be
    /// DataType::Float.
    ///
    ...
    /// DataType::Int8.
    ///
    CNTK_API void SetValue(char value);

    ///
    /// Creates a new NDArrayView with newly allocated storage on the specified device and copies 'this' view's contents
    /// into the newly allocated view.
    ///
    CNTK_API NDArrayViewPtr DeepClone(const DeviceDescriptor& device, bool readOnly = false) const;

    ///
    /// Creates a new NDArrayView with newly allocated storage on the same device as 'this' view and copies 'this'
    /// view's contents into the newly allocated view.
    ///
    ...
    ///
    /// Creates a new NDArrayView which is an alias of 'this' view; i.e. a new view of the same shape as 'this' over the
    ...
    ///
    CNTK_API NDArrayViewPtr SliceView(const std::vector<size_t>& startOffset, const std::vector<size_t>& extent,
    ...
    ///
    /// Static method to construct a new NDArrayView object whose contents are drawn from a normal distribution with the
    /// specified mean and standard deviation..
    ///
    template <typename ElementType>
    CNTK_API static NDArrayViewPtr RandomNormal(const NDShape& shape, double mean, double stdDev,
                                                unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                                const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice());

    ///
    /// Static method to construct a new NDArrayView object whose contents are drawn from a uniform distribution in the
    /// specified value range.
    ///
    template <typename ElementType>
    CNTK_API static NDArrayViewPtr RandomUniform(const NDShape& shape, double rangeStart, double rangeEnd,
                                                 unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                                 const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice());
...
private:
    ...
    template <typename ElementType, typename V1ElemType>
    std::tuple<const ElementType*, const SparseIndexType*, const SparseIndexType*, size_t>
    _SparseCSCDataBuffers() const;

    template <typename ElementType, typename V1ElemType>
    std::tuple<const void*, const SparseIndexType*, const SparseIndexType*, size_t, size_t, size_t>
    _SparseBlockColumnDataBuffers() const;

    template <typename ElementType, typename V1ElemType>
    static NDArrayViewPtr _RandomNormal(const NDShape& shape, double mean, double stdDev, unsigned long seed,
                                        const DeviceDescriptor& device);

    template <typename ElementType, typename V1ElemType>
    static NDArrayViewPtr _RandomUniform(const NDShape& shape, double rangeStart, double rangeEnd, unsigned long seed,
                                         const DeviceDescriptor& device);
...
private:
    ...
    template <typename ElementType>
    static std::shared_ptr<Microsoft::MSR::CNTK::Matrix<ElementType>>
    GetMatrixImpl(const Microsoft::MSR::CNTK::TensorView<ElementType>* tensorView, size_t rowColSplitPoint);

    template <typename ElementType>
    std::shared_ptr<const Microsoft::MSR::CNTK::Matrix<ElementType>>
    GetMatrix(size_t rowColSplitPoint = AutoSelectRowColSplitPoint) const;

    template <typename ElementType>
    std::shared_ptr<Microsoft::MSR::CNTK::Matrix<ElementType>>
    GetWritableMatrix(size_t rowColSplitPoint = AutoSelectRowColSplitPoint);

    std::shared_ptr<const Microsoft::MSR::CNTK::MatrixBase>
    GetMatrixBase(size_t rowColSplitPoint = AutoSelectRowColSplitPoint) const;

    std::shared_ptr<Microsoft::MSR::CNTK::MatrixBase>
    GetWritableMatrixBase(size_t rowColSplitPoint = AutoSelectRowColSplitPoint);
...
public:
    ...
    ///
    /// Creates a new NDMask with newly allocated storage on the specified device and copies 'this' mask's contents into
    /// the newly allocated view.
    ///
    CNTK_API NDMaskPtr DeepClone(const DeviceDescriptor& device) const;

    ///
    /// Creates a new NDMask with newly allocated storage on the same device as 'this' mask and copies 'this' mask's
    /// contents into the newly allocated mask.
    ///
...
private:
    ...
    CNTK_API void MarkSectionAs(const std::vector<size_t>& sectionOffset, const NDShape& sectionShape,
                                MaskKind maskKind);
...
///
/// Denotes an Axis of a Variable and is used for specifying the axes parameters of certain Functions such as
/// reductions. Besides the static axes corresponding to each of the axes of the Variable's shape, Variables of kind
/// 'Input' and any 'Output' Variables dependent on an 'Input' Variable also have 2 additional dynamic axes whose
/// dimensions are known only when the Variable is bound to actual data during compute (viz. sequence axis and batch
/// axis denoting the axis along which multiple sequences are batched)
///
class Axis final
{
...
public:
    ...
    ///
    explicit Axis(int staticAxisIdx) : m_staticAxisIdx(staticAxisIdx), m_isOrderedDynamicAxis(false)
    {
        if (IsStaticAxis())
    ...
    explicit Axis(const std::wstring& name, bool isOrderedDynamicAxis = true)
        : m_staticAxisIdx(SentinelStaticAxisIndexValueForDynamicAxes),
          m_name(name),
          m_isOrderedDynamicAxis(isOrderedDynamicAxis)
    {
    ...
    ///
    /// Returns a boolean indicating if 'this' Axis is ordered; i.e. if there is an ordering between the dimensions
    /// along this axis.
    ///
    bool IsOrdered() const
    {
        return IsStaticAxis() || m_isOrderedDynamicAxis;
    }
    ...
    ///
    Axis() : m_staticAxisIdx(SentinelStaticAxisIndexValueForDynamicAxes) {}
...
public:
    ...
    DictionaryValue(const wchar_t* value) : DictionaryValue(std::wstring(value)) {}
    ...
    DictionaryValue(const std::vector<::CNTK::DictionaryValue>& value)
        : m_valueType(GetValueType<std::vector<::CNTK::DictionaryValue>>())
    {
    ...
    {
        static_assert(
            (std::is_same<T, NDShape>::value || std::is_same<T, Axis>::value || std::is_same<T, std::wstring>::value ||
             std::is_same<T, std::vector<DictionaryValue>>::value || std::is_same<T, Dictionary>::value ||
             std::is_same<T, TrainingParameterSchedule<double>>::value || std::is_same<T, NDArrayView>::value),
    ...
    DictionaryValue& operator=(DictionaryValue&& other)
    {
        ...
        if (other.m_valueType == Type::String || other.m_valueType == Type::NDShape ||
            other.m_valueType == Type::Axis || other.m_valueType == Type::Vector ||
            other.m_valueType == Type::Dictionary || other.m_valueType == Type::TrainingParameterSchedule ||
    ...
    template <typename T,
              typename std::enable_if<
                  std::is_same<T, NDShape>::value || std::is_same<T, Axis>::value ||
                  std::is_same<T, std::wstring>::value || std::is_same<T, std::vector<DictionaryValue>>::value ||
                  std::is_same<T, Dictionary>::value || std::is_same<T, TrainingParameterSchedule<double>>::value ||
...
private:
    ...
    {
        static_assert(
            (std::is_same<T, bool>::value || std::is_same<T, int>::value || std::is_same<T, size_t>::value ||
             std::is_same<T, float>::value || std::is_same<T, double>::value || std::is_same<T, std::wstring>::value ||
             std::is_same<T, NDShape>::value || std::is_same<T, Axis>::value ||
             std::is_same<T, std::vector<DictionaryValue>>::value || std::is_same<T, Dictionary>::value ||
             std::is_same<T, TrainingParameterSchedule<double>>::value || std::is_same<T, NDArrayView>::value),
            "Unsupported ValueType");
        ...
        if (std::is_same<T, std::wstring>::value)
            return Type::String;
        if (std::is_same<T, NDShape>::value)
            return Type::NDShape;
        if (std::is_same<T, Axis>::value)
            return Type::Axis;
        if (std::is_same<T, std::vector<DictionaryValue>>::value)
            return Type::Vector;
        if (std::is_same<T, Dictionary>::value)
            return Type::Dictionary;
        if (std::is_same<T, NDArrayView>::value)
            return Type::NDArrayView;
        if (std::is_same<T, TrainingParameterSchedule<double>>::value)
            return Type::TrainingParameterSchedule;
    }
    ...
    void VerifyType() const
    {
        if (GetValueType<T>() != m_valueType)
            RuntimeError("Reading a DictionaryValue as the wrong type; Reading as type %s when actual type is %s",
                         typeid(T).name(), DictionaryValue::TypeName(m_valueType));
    }
...
class Dictionary
{
    friend inline void AddConfigString(std::wstringstream& s, const DictionaryValue& value,
                                       size_t numIndentationSpaces);
...
public:
    ...
    {
        return m_dictionaryData->begin();
    }
    ConstDictionaryIterator cbegin() const
    {
        return m_dictionaryData->cbegin();
    }
    DictionaryIterator end() const
    {
        return m_dictionaryData->end();
    }
    ConstDictionaryIterator cend() const
    {
        return m_dictionaryData->cend();
    }

    size_t Size() const
    {
        return m_dictionaryData->size();
    }
...
inline Variable PlaceholderVariable(const NDShape& shape, ::CNTK::DataType dataType, const std::wstring& name,
                                    const std::vector<Axis>& dynamicAxes = Axis::UnknownDynamicAxes());
inline Variable InputVariable(const NDShape& shape, bool isSparse, ::CNTK::DataType dataType, bool needsGradient,
                              const std::wstring& name,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes());
inline Variable OutputVariable(const NDShape& shape, ::CNTK::DataType dataType, const std::vector<Axis>& dynamicAxes,
                               bool needsGradient, const std::wstring& name = L"");
...
#ifndef SWIG
private:
    friend inline Variable PlaceholderVariable(const NDShape& shape, ::CNTK::DataType dataType,
                                               const std::wstring& name, const std::vector<Axis>& dynamicAxes);
    friend inline Variable
    InputVariable(const NDShape& shape, bool isSparse, ::CNTK::DataType dataType, bool needsGradient,
                  const std::wstring& name,
                  const std::vector<Axis>& dynamicAxes /*= Axis::DefaultInputVariableDynamicAxes()*/);
    friend inline Variable OutputVariable(const NDShape& shape, ::CNTK::DataType dataType,
                                          const std::vector<Axis>& dynamicAxes, bool needsGradient,
...
public:
    ...
    bool IsInput() const
    {
        return Kind() == VariableKind::Input;
    }
    ...
    bool IsOutput() const
    {
        return Kind() == VariableKind::Output;
    }
    ...
    /// Timestamps are used to determine if a Variable's value is up to date and, if not, computations that depend on
    /// this Variable's value will be re-executed.
    ///
...
public:
#endif
    Variable(const NDShape& shape, VariableKind varType, ::CNTK::DataType dataType, const NDArrayViewPtr& value,
             bool needsGradient, const std::vector<Axis>& dynamicAxes, const std::wstring& name,
...
public:
#endif
    Variable(const NDShape& shape, bool isSparse, ::CNTK::DataType dataType, bool needsGradient,
             const std::wstring& name, const std::vector<Axis>& dynamicAxes, const std::wstring& uid)
        : Variable(shape, VariableKind::Input, dataType, nullptr, needsGradient, dynamicAxes, isSparse, name, uid)
    {
    }

    // TODO: This should be a private but if not made public, the python bindings build complains about an unresolved
    // external Probably due the above ctor being a public method in SWIG codegen
public:
    CNTK_API Variable(const NDShape& shape, VariableKind varType, ::CNTK::DataType dataType,
                      const NDArrayViewPtr& value, bool needsGradient, const std::vector<Axis>& dynamicAxes,
                      bool isSparse, const std::wstring& name, const std::wstring& uid);
...
private:
    ...
    template <typename ElementType>
    static NDArrayViewPtr CreateValueFromParameterInitializer(const NDShape& shape,
                                                              const ParameterInitializer& initConfig,
                                                              const DeviceDescriptor& device);

    CNTK_API static Variable Deserialize(const Dictionary& dictionary,
...
/// All placeholder inputs of a Function must be replaced with non-placeholder Variables before Forward evaluation of
/// the Function.
///
inline Variable PlaceholderVariable(const NDShape& shape, ::CNTK::DataType dataType, const std::wstring& name,
...
/// the Function.
///
inline Variable PlaceholderVariable(const NDShape& shape, const std::wstring& name,
                                    const std::vector<Axis>& dynamicAxes)
{
...
/// the Function.
///
inline Variable PlaceholderVariable(const NDShape& shape,
                                    const std::vector<Axis>& dynamicAxes = Axis::UnknownDynamicAxes())
{
                              ...
                              const std::vector<Axis>& dynamicAxes /*= Axis::DefaultInputVariableDynamicAxes()*/)
{
    return Variable(shape, isSparse, dataType, needsGradient, name, dynamicAxes,
                    Internal::GenerateUid(VariableKind::Input));
}

///
/// Create an 'Input' Variable and specify if gradients are to be computed for this input
///
inline Variable InputVariable(const NDShape& shape, ::CNTK::DataType dataType, bool needsGradient,
                              const std::wstring& name = L"",
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
///
inline Variable InputVariable(const NDShape& shape, DataType dataType, const std::wstring& name,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
/// Create an 'Input' Variable.
///
inline Variable InputVariable(const NDShape& shape, DataType dataType, const wchar_t* name,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
/// Create an 'Input' Variable.
///
inline Variable InputVariable(const NDShape& shape, DataType dataType,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
///
inline Variable InputVariable(const NDShape& shape, bool isSparse, ::CNTK::DataType dataType, const std::wstring& name,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
/// Create an 'Input' Variable denoting sparse data.
///
inline Variable InputVariable(const NDShape& shape, bool isSparse, ::CNTK::DataType dataType, const wchar_t* name,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
/// Create an 'Input' Variable denoting sparse data.
///
inline Variable InputVariable(const NDShape& shape, bool isSparse, ::CNTK::DataType dataType,
                              const std::vector<Axis>& dynamicAxes = Axis::DefaultInputVariableDynamicAxes())
{
...
inline Variable OutputVariable(const NDShape& shape, ::CNTK::DataType dataType, const std::vector<Axis>& dynamicAxes,
                               bool needsGradient, const std::wstring& name /*= L""*/)
{
    return Variable(shape, VariableKind::Output, dataType, nullptr, needsGradient, dynamicAxes, /*isSparse =*/false,
                    name, Internal::GenerateUid(VariableKind::Output));
}
...
CNTK_API ParameterInitializer UniformInitializer(double scale,
                                                 unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer NormalInitializer(double scale, int outputRank = SentinelValueForInferParamInitRank,
                                                int filterRank = SentinelValueForInferParamInitRank,
                                                unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer XavierInitializer(double scale = DefaultParamInitScale,
                                                int outputRank = SentinelValueForInferParamInitRank,
                                                int filterRank = SentinelValueForInferParamInitRank,
                                                unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer GlorotUniformInitializer(double scale = DefaultParamInitScale,
                                                       int outputRank = SentinelValueForInferParamInitRank,
                                                       int filterRank = SentinelValueForInferParamInitRank,
                                                       unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer GlorotNormalInitializer(double scale = DefaultParamInitScale,
                                                      int outputRank = SentinelValueForInferParamInitRank,
                                                      int filterRank = SentinelValueForInferParamInitRank,
                                                      unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer HeUniformInitializer(double scale = DefaultParamInitScale,
                                                   int outputRank = SentinelValueForInferParamInitRank,
                                                   int filterRank = SentinelValueForInferParamInitRank,
                                                   unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer HeNormalInitializer(double scale = DefaultParamInitScale,
                                                  int outputRank = SentinelValueForInferParamInitRank,
                                                  int filterRank = SentinelValueForInferParamInitRank,
                                                  unsigned long seed = SentinelValueForAutoSelectRandomSeed);
CNTK_API ParameterInitializer BilinearInitializer(size_t kernelWidth, size_t kernelHeight);
CNTK_API ParameterInitializer RandomInitializerWithRank(const ParameterInitializer& initializer, int outputRank,
                                                        int filterRank);
CNTK_API ParameterInitializer TruncatedNormalInitializer(double scale = DefaultParamInitScale,
                                                         unsigned long seed = SentinelValueForAutoSelectRandomSeed);
...
public:
    ...
    Parameter(const NDShape& shape, ElemType initValue,
              const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(), const std::wstring& name = L"")
    ...
    ///
    Parameter(const NDShape& shape, DataType dataType, double initValue,
              const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(), const std::wstring& name = L"")
    ...
    ///
    CNTK_API Parameter(const NDShape& shape, DataType dataType, const ParameterInitializer& initializer,
                       const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(),
                       const std::wstring& name = L"");

    ///
    /// DownCast a Variable to a Parameter. Only allowed if the VariableKind is Parameter and throws an exception
    /// otherwise.
    ///
    explicit Parameter(const Variable& variable) : Variable(variable)
    {
        if (!IsParameter())
            InvalidArgument("A non-parameter Variable '%S' cannot be converted to a Parameter.",
                            variable.AsString().c_str());
    }
...
private:
    ...
    {
        if (value->IsReadOnly())
            InvalidArgument("Parameter cannot be constructed from a read-only NDArrayView value; you can create a non "
                            "read-only clone of the value and use that instead!");
    }
};

// Implementation note: The Variable type is a value type and not polymorphic in nature.
// However we have a couple of derivatives of the type to extend the base interface and thus we ensure that the derived
// types do not have additional fields. This check is weak in that the derives types may sneak in some additional fields
// if the base type had some padding at the end, without changing the object size but it should be good enough for
// catching any accidental addition of fields.
static_assert(sizeof(Parameter) == sizeof(Variable),
              "The Parameter type should not have any data fields beyond what its base type 'Variable' has.");
...
public:
    ...
    ///
    template <typename ElemType>
    Constant(const NDShape& shape, ElemType initValue,
             const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(), const std::wstring& name = L"")
    ...
    /// Construct a constant of specified shape whose contents are initialized with the specified 'initValue'
    ///
    Constant(const NDShape& shape, DataType dataType, double initValue,
             const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(), const std::wstring& name = L"")
    ...
    /// This only supports converting from a lower precision type to a higher precision type (e.g. DataType::Float to
    /// DataType::Double)
    ///
    ...
    ///
    static inline ::CNTK::Constant Scalar(::CNTK::DataType dataType, double value,
                                          const ::CNTK::DeviceDescriptor& device = DeviceDescriptor::CPUDevice())
    {
    ...
    ///
    template <typename ElementType>
    static inline ::CNTK::Constant Scalar(ElementType value,
                                          const ::CNTK::DeviceDescriptor& device = DeviceDescriptor::CPUDevice())
    {
    ...
    ///
    /// DownCast a Variable to a Constant. Only allowed if the VariableKind is Constant and throws an exception
    /// otherwise.
    ///
    explicit Constant(const Variable& variable) : Variable(variable)
    {
        if (!IsConstant())
...
private:
    ...
    /// Construct a constant of specified shape whose contents are initialized using the specified initializer
    ///
    CNTK_API Constant(const NDShape& shape, DataType dataType, const ParameterInitializer& initializer,
                      const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice(),
                      const std::wstring& name = L"");
};
...
// if the base type had some padding at the end, without changing the object size but it should be good enough for
// catching any accidental addition of fields.
static_assert(sizeof(Constant) == sizeof(Variable),
              "The Constant type should not have any data fields beyond what its base type 'Variable' has.");
} // namespace CNTK
...
{
///
/// Denotes a multi-dimensional array with an optional mask and is the actual data fed into or produced from a
/// computation. The mask is typically lower dimensionality than the data, meaning data is masked in coarse individual
/// sample units where sample shape is data.Shape().SubShape(0, data.Shape().Rank() - mask.Shape().Rank) Also, note that
/// the size of the data's trailing mask.Shape().Rank() dimensions must match the mask shape dimensions.
///
...
public:
    ...
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr Create(const NDShape& sampleShape, const std::vector<std::vector<ElementType>>& sequences,
                                    const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                                    bool readOnly = false);
    ...
    ///
    template <typename ElementType>
    static ValuePtr Create(const NDShape& sampleShape, const std::vector<std::vector<ElementType>>& sequences,
                           const DeviceDescriptor& device, bool readOnly = false)
    {
    ...
    /// Create a new Value object containing a collection of variable length sequences.
    ///
    CNTK_API static ValuePtr Create(const NDShape& sampleShape, const std::vector<NDArrayViewPtr>& sequences,
                                    const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                                    bool readOnly, bool createNewCopy);

    ///
    /// Create a new Value object containing a collection of variable length sequences.
    /// The created Value object contains a copy of the specified 'sequences' data.
    ///
    static ValuePtr Create(const NDShape& sampleShape, const std::vector<NDArrayViewPtr>& sequences,
                           const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                           bool readOnly = false)
    {
    ...
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr Create(const NDShape& sampleShape, const std::vector<std::vector<size_t>>& oneHotSequences,
                                    const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                                    bool readOnly = false);
    ...
    ///
    template <typename ElementType>
    static ValuePtr Create(const NDShape& sampleShape, const std::vector<std::vector<size_t>>& oneHotSequences,
                           const DeviceDescriptor& device, bool readOnly = false)
    {
    ...
    ///
    template <typename ElementType>
    static ValuePtr Create(size_t dimension, const std::vector<std::vector<size_t>>& oneHotSequences,
                           const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                           bool readOnly = false)
    {
    ...
    /// The number of samples in the batch is the number of elements in batch divided by the size of shape (A runtime
    /// error occurs if the remainder is not zero). The created Value object contains a copy of the specified data in
    /// batch. Parameters:
    ...
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr CreateBatch(const NDShape& sampleShape, const std::vector<ElementType>& batchData,
                                         const DeviceDescriptor& device, bool readOnly = false);
    ...
    /// The sequenceStartFlag specifies wehther this sequence is a new sequence or continuation of a previous sequence
    /// at the same index in the sequences list from a previous call to this method.The sequence length is the number of
    /// elements in sequence divided by the size of shape. (A runtime error occurs if the remainder is not zero).
    /// Parameters:
    ///     sampleShape: the tensor shape of the Value.
    ///     sequenceData: the data to be contained in the Value.
    ///     sequenceStartFlag: true indicates that it is a new sequence. false means a continuation of a previous
    ///     sequence. device: on which device the Value object should be created. readOnly: the Value is read-only if
    ///     this flag is true.
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr CreateSequence(const NDShape& sampleShape, const std::vector<ElementType>& sequenceData,
                                            bool sequenceStartFlag, const DeviceDescriptor& device,
                                            bool readOnly = false);

    ///
    /// Creates a new Value object containing a sequence of samples.
    /// The created Value object contains a copy of the specified data in sequence.
    /// The sequence length is the number of elements in sequence divided by the size of shape(A runtime error occurs if
    /// the remainder is not zero). The created sequece is a new sequence. Parameters:
    ///     sampleShape: the tensor shape of the Value.
    ///     sequenceData: the data to be contained in the Value.
    ///     device: on which device the Value object should be created.
    ...
    ///
    template <typename ElementType>
    static ValuePtr CreateSequence(const NDShape& sampleShape, const std::vector<ElementType>& sequenceData,
                                   const DeviceDescriptor& device, bool readOnly = false)
    {
    ...
    /// The length of each sequence is the number of elements in the corresponding sequence of batchOfSequences divided
    /// by the size of shape. (A runtime error occurs if the remainder is not zero). Parameters:
    ///     sampleShape: the tensor shape of the Value.
    ///     batchOfSequences: the data to be stored in the Value.The outer vector represents a collection of sequences
    ///     with variable length, and the inner vector represents each individual sequence. sequenceStartFlags: A
    ///     collection of boolean value. Each element represent whether the correspoinding sequence in batchOfSequences
    ///     is a new sequence (in case of true) or a continuation of a previous sequence (in case of false). device: on
    ///     which device the Value should be created. readOnly: the Value is read-only if this flag is true.
    ///
    template <typename ElementType>
    static ValuePtr CreateBatchOfSequences(const NDShape& sampleShape,
                                           const std::vector<std::vector<ElementType>>& batchOfSequences,
                                           const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                                           bool readOnly = false)
    {
    ...
    /// The number of sequences in the batch is the size of batchOfSequences.
    /// The length of each sequence is the number of elements in the corresponding sequence of batchOfSequences divided
    /// by the size of shape. (A runtime error occurs if the remainder is not zero). Each sequence in batchOfSequences
    /// is a new sequence. Parameters:
    ///     sampleShape: the tensor shape of the Value.
    ///     batchOfSequences: the data to be stored in the Value.The outer vector represents a collection of sequences
    ///     with variable length, and the inner vector represents each individual sequence. device: on which device the
    ///     Value should be created. readOnly: the Value is read-only if this flag is true.
    ///
    template <typename ElementType>
    static ValuePtr CreateBatchOfSequences(const NDShape& sampleShape,
                                           const std::vector<std::vector<ElementType>>& batchOfSequences,
    ...
    ///
    /// Creates a new Value object containing a batch of samples.
    /// Each sample is represented by an index value that points to the non-zero value in the one-hot vector of
    /// dimension elements. The number of samples in the batch is the number of elements in batch. Parameters:
    ...
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr CreateBatch(size_t dimension, const std::vector<size_t>& batchData,
                                         const DeviceDescriptor& device, bool readOnly = false);

    ///
    /// Creates a new Value object containing a sequence of samples.
    /// Each sample is represented by an index value that points to the non-zero value in the one-hot vector of
    /// dimension elements. The sequenceStartFlag specifies wehther this sequence is a new sequence or continuation of a
    /// previous sequence at the same index in the sequences list from a previous call to this method. The sequence
    /// length is the number of elements in sequence. Parameters:
    ///     ElementType: data type of the created Value object.Currently, float and double are supported.
    ///     dimension: the size of dimension of the one-hot vector.
    ...
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr CreateSequence(size_t dimension, const std::vector<size_t>& sequenceData,
                                            bool sequenceStartFlag, const DeviceDescriptor& device,
                                            bool readOnly = false);
    ...
    /// Creates a new Value object containing a sequence of samples.
    /// Each sample is represented by an index value that points to the non-zero value in the one-hot vector of
    /// dimension elements. The sequence length is the number of elements in sequence. The created sequence is a new
    /// sequence. Parameters:
    ///     ElementType: data type of the created Value object.Currently, float and double are supported.
    ///     dimension: the size of dimension of the one-hot vector.
    ///     sequenceData: the collection of indexes representing the sequence of samples.
    ...
    ///
    template <typename ElementType>
    static ValuePtr CreateSequence(size_t dimension, const std::vector<size_t>& sequenceData,
                                   const DeviceDescriptor& device, bool readOnly = false)
    {
    ...
    /// Creates a new Value object containing a batch of variable length sequences.
    /// Each sample is represented by an index value that points to the non-zero value in the one-hot vector of
    /// dimension elements. The number of sequences is the number of elements in the outer vector of batchOfSequences.
    /// The length of each sequence is the number of elements of the corresponding sequence in the inner vector of
    /// batchOfSequences. Parameters:
    ///     ElementType: data type of the created Value object.Currently, float and double are supported.
    ///     dimension: the size of dimension of the one-hot vector.
    ///     batchOfSequences: the collection of indexes representing sequences of samples.The outer vector represents a
    ///     collection of sequences with variable length, and the inner vector represents each individual sequence.
    ///     sequenceStartFlags: A collection of boolean value.Each element represent whether the correspoinding sequence
    ///     in batchOfSequences is a new sequence(in case of true) or a continuation of a previous sequence(in case of
    ///     false). device: on which device the Value object should be created. readOnly: the Value is read-only if this
    ///     flag is true.
    ///
    template <typename ElementType>
    static ValuePtr CreateBatchOfSequences(size_t dimension, const std::vector<std::vector<size_t>>& batchOfSequences,
                                           const std::vector<bool>& sequenceStartFlags, const DeviceDescriptor& device,
                                           bool readOnly = false)
    {
    ...
    /// dimension elements. The number of sequences is the number of elements in the outer vector of batchOfSequences.
    /// The length of each sequence is the number of elements of the corresponding sequence in the inner vector of
    /// batchOfSequences. Each sequence in batchOfSequences is a new sequence. Parameters:
    ///     ElementType: data type of the created Value object.Currently, float and double are supported.
    ///     dimension: the size of dimension of the one-hot vector.
    ///     batchOfSequences: the collection of indexes representing sequences of samples.The outer vector represents a
    ///     collection of sequences with variable length, and the inner vector represents each individual sequence.
    ...
    ///
    /// Creates a new Value object containing a sequence of samples.
    /// The sequence is represented by CSC sparse input format
    /// (http://docs.nvidia.com/cuda/cusparse/#compressed-sparse-column-format-csc) The sequenceStartFlag specifies
    /// wehther this sequence is a new sequence or continuation of a previous sequence at the same index in the
    /// sequences list from a previous call to this method. The sequence length is determined by the number of rows of
    /// the sparse matrix. Parameters:
    ///     ElementType: data type of the created Value object.Currently, float and double are supported.
    ///     sampleShape: the tensor shape. For sparse input, the tensor shape leading dimensionality must be the same as
    ///     the total size of the tensor shape. sequenceLength: the sequence length. sequenceData: the collection of
    ///     indexes representing the sequence of samples. sequenceStartFlag : true indicates that it is a new sequence.
    ///     false means a continuation of a previous sequence. device : on which device the Value object should be
    ///     created. readOnly : the Value is read - only if this flag is true.
    ///
    template <typename ElementType>
    CNTK_API static ValuePtr
    CreateSequence(const NDShape& sampleShape, size_t sequenceLength, const SparseIndexType* colStarts,
                   const SparseIndexType* rowIndices, const ElementType* nonZeroValues, size_t numNonZeroValues,
    ...
    ///
    template <typename ElementType>
    static ValuePtr CreateSequence(const NDShape& sampleShape, size_t sequenceLength, const SparseIndexType* colStarts,
                                   const SparseIndexType* rowIndices, const ElementType* nonZeroValues,
                                   size_t numNonZeroValues, const DeviceDescriptor& device, bool readOnly = false)
    {
        return CreateSequence(sampleShape, sequenceLength, colStarts, rowIndices, nonZeroValues, numNonZeroValues, true,
                              device, readOnly);
    }

    template <typename ElementType>
    static ValuePtr CreateSequence(size_t dimension, size_t sequenceLength, const SparseIndexType* colStarts,
                                   const SparseIndexType* rowIndices, const ElementType* nonZeroValues,
                                   size_t numNonZeroValues, bool sequenceStartFlag, const DeviceDescriptor& device,
                                   bool readOnly = false)
    {
        auto sampleShape = NDShape({dimension});
        return CreateSequence(sampleShape, sequenceLength, colStarts, rowIndices, nonZeroValues, numNonZeroValues,
                              sequenceStartFlag, device, readOnly);
    }
    ...
    ///
    /// Creates a new Value with newly allocated storage on the same device as 'this' Value and copies 'this' Value's
    /// contents into the newly allocated Value.
    ///
    ...
    ///
    std::pair<std::vector<NDArrayViewPtr>, std::vector<bool>>
    UnpackVariableValue(const Variable& variable, bool sequenceSegmentsAllowed, const DeviceDescriptor& device)
    {
        ...
        auto valueShapeWithSequenceAndBatchAxes =
            actualVariableShape.AppendShape(NDShape({maxSequenceLen, numOfSequences}));
        ...
        for (size_t i = 0; i < numOfSequences; ++i)
        {
            if (!sequenceSegmentsAllowed && (sequenceBeginIndices[i] != 0))
                RuntimeError("Value::UnpackVariableValue: Only Value objects containing the entire sequence (no "
    ...
    /// and the colStarts, rowIndices and nonZeroValues contain the data of column indexes, row indexes and non-zero
    /// values, and the numNonZeroValues is set to number of non-zero values contained in 'this' Value.
    ///
    template <typename ElementType>
    void CopyVariableValueTo(const Variable& outputVariable, size_t& sequenceLength,
                             std::vector<SparseIndexType>& colStarts, std::vector<SparseIndexType>& rowIndices,
                             std::vector<ElementType>& nonZeroValues, size_t& numNonZeroValues)
    {
        size_t numColsInMatrix;
        std::tie(sequenceLength, numColsInMatrix, numNonZeroValues) =
            ValidateSparseCSCAndGetIndexBufferSizes<ElementType>(outputVariable);
...
private:
    template <typename ElementType>
    static void AppendSparseSequenceData(const NDArrayViewPtr& sequenceData, std::vector<SparseIndexType>& colStarts,
                                         std::vector<SparseIndexType>& rowIndices, std::vector<char>& nonZeroValues,
                                         size_t maxSequenceLengthInCols);

    ///
    /// Copy the data stored in 'this' Value object to the buffer 'sequences' as a collection of variable length
    /// sequences. The output data is in the dense format. Assumption: The 'sequences' buffer has been resized to match
    /// the number of sequences and the length of each sequence stored in the Value object. The resizing is done by
    /// ResizeOutputBuffer() and needs to be done on the heap of the caller.
    ///
    template <typename ElementType>
    ...
    ///
    /// Copy the data stored in 'this' Value object to the buffer 'sequences' as a collection of variable length
    /// sequences. The output data is in the one-hot format. The resizing is done by ResizeOutputBuffer() and needs to
    /// be done on the heap of the caller. Assumption: The 'sequences' buffer has been resized to match the number of
    /// sequences and the length of each sequence stored in the Value object.
    ///
    template <typename ElementType>
...
private:
    CNTK_API std::pair<size_t, size_t> GetSequenceAndBatchLength(const Variable& outputVariable,
                                                                 NDShape* inferredVarShape = nullptr);

    template <typename ElementType>
    CNTK_API std::tuple<size_t, size_t, size_t> ValidateSparseCSCAndGetIndexBufferSizes(const Variable& outputVariable);

    template <typename ElementType>
    CNTK_API void CopyVariableValueToCSCSparse(size_t sequenceLength, std::vector<SparseIndexType>& colStarts,
                                               std::vector<SparseIndexType>& rowIndices,
                                               std::vector<ElementType>& nonZeroValues, size_t& numNonZeroValues);

    CNTK_API static void GetSequenceStartsAndLengths(const NDMaskPtr& mask,
                                                     std::vector<ptrdiff_t>& sequenceBeginIndices,
                                                     std::vector<size_t>& sequenceLengths, size_t numDynamicAxes);

    ///
    /// Resize the 'sequences' buffer if needed.
    /// It should be kept in the header file, as the memory should be allocated at the caller side, not the
    /// CNTKLibarary.dll side. outputVariable defines tensor, the sequence axis and the batch axis. The 'sequences' is
    /// the output buffer which is used to store data from 'this' Value. On return, its size and the size of its each
    /// element are adjusted to match the number of sequences and the length of each sequence stored in the Value
    /// object.
    ///
    template <typename ElementType>
    void ResizeOutputBuffer(const Variable& outputVariable, std::vector<std::vector<ElementType>>& sequences)
    {
        if (outputVariable.Shape().IsUnknown())
            RuntimeError("The outputVariable '%S' shape '%S' is unknown shape.", outputVariable.AsString().c_str(),
                         outputVariable.Shape().AsString().c_str());

        // Make sure that inferredVarShape has correct rank in order to avoid any rank resize during inferring free
        // dimension.
        NDShape inferredVarShape = outputVariable.Shape();
        size_t numOfSequences;
        size_t maxSequenceLen;
        // Verify compatibility of 'this' value and outputVariable, get sequence and batch length, and get the inferred
        // shape if the variable has a free dimension.
...
public:
    ...
    /// The function and computeDevice parameters record the Function and compute device that 'this' BackPropState
    /// corresponds to The forwardPropValuesToSave is an optional map of forward compute values saved for later use
    /// during back propagation of gradients in the backward call that 'this' BackPropState object is used.
    ///
    BackPropState(const FunctionPtr& function, const DeviceDescriptor& computeDevice,
                  const std::unordered_map<Variable, ValuePtr>& forwardPropValuesToSave = {})
    ...
    ///
    const std::unordered_map<Variable, ValuePtr>& SavedForwardPropValues() const
    {
        return m_savedForwardPropValues;
    }
...
///
typedef std::function<FunctionPtr(const std::vector<Variable>& /*inputs*/, const std::wstring& /*name*/,
                                  const Dictionary& /*dictionary*/)>
    UDFDeserializeCallback;

typedef std::shared_ptr<UDFDeserializeCallback> UDFDeserializeCallbackPtr;

static auto NoOp = [](const std::vector<Variable>&, const std::wstring&, const Dictionary&) { return nullptr; };
...
/// A Function may be primitive or composite (comprised of other Function instances whose inputs and outputs are wired
/// together). A Function effectively is a computation graph composed of other primitive Functions (denoting
/// computation) as nodes and Variable objects (denoting data) as the edges and leaves of the graph. Function class
/// inherits from  IDictionarySerializable to allow derived 'Function' types to specify custom serialization procedure.
///
...
public:
    ///
    /// Computes and stores the values of specified variables in the 'outputs' map, using provided 'inputs' values
    /// corresponding to each leaf variable of the Function of VariableKind 'Input'. The variables specified in the
    /// 'outputs' map denote the subset of 'this' Function's output variables that the caller wants to obtain values of.
    /// Callers may specify the storage to be used for storing the 'outputs' Values or pass null in which case the
    /// implementation allocates the actual storage for the 'outputs' for which the ValuePtr mapping was left null by
    /// the caller. If a null Value was specified, the implementation created Value objects are temporary and only
    /// guaranteed to be valid until the next Forward/Backward call. You must explicitly clone the temporay Values if
    /// they need to be accessed later. The optional 'outputsToRetainBackwardStateFor' parameter specifies the subset of
    /// the Function's output variables for which gradients will be specified in a subsequent Backward call for
    /// backpropagation. The method returns a BackPropState object containing all intermediate variable values needed
    /// during backpropagation of gradients from the 'outputsToRetainBackwardStateFor' outputs of the Function to any of
    /// the inputs of the Function, in a subsequent Backward call. Note that the returned BackPropState instance also
    /// stores a reference to the supplied 'inputs' Values and generated 'outputs' Values and the user is responsible
    /// for ensuring that the contents of the inputs and outputs are unchanged until after any uses of the BackPropState
    /// instance for backpropagating gradients through this Function.
    ///
    ...
    ///
    /// Backpropagates supplied 'rootGradientValues' for one or more of the output variables of the Function, to produce
    /// gradient Values corresponding to the specified set of input variables in
    /// 'backPropagatedGradientValuesForInputs'. Callers may specify the actual storage to be used for storing the
    /// 'backPropagatedGradientValuesForInputs' Values or leave them to be null in which case the implementation
    /// allocates the actual storage for storing the gradients. If a null Value was specified, the implementation
    /// created Value objects are temporary and only guaranteed to be valid until the next Forward/Backward call. You
    /// must explicitly clone the temporay Values if they need to be accessed later. In case an existing storage is
    /// specified, the gradients are aggregated with existing values in the specified storage. The 'state' parameter is
    /// an instance of an BackPropState instance obtained from a previous call to the Forward method on 'this; Function
    /// for the computation that this gradient backpropagation corresponds to.
    ///
...
protected:
    ///
    /// Computes and stores the values of specified variables in the 'outputs' map, using provided 'inputs' values for
    /// each input of the Function. The variables specified in the 'outputs' map denote the subset of 'this' Function's
    /// output variables that the caller wants to obtain values of. Callers may specify the storage to be used for
    /// storing the 'outputs' Values or pass null in which case the implementation allocates the actual storage for the
    /// 'outputs' for which the ValuePtr mapping was left null by the caller.  If a null Value was specified, the
    /// implementation created Value objects are temporary and only guaranteed to be valid until the next
    /// Forward/Backward call. You must explicitly clone the temporay Values if they need to be accessed later. The
    /// optional 'outputsToRetainBackwardStateFor' parameter specifies the subset of the Function's output variables for
    /// which gradients will be specified in a subsequent Backward call for backpropagation. The method returns a
    /// BackPropState object containing all intermediate variable values needed during backpropagation of gradients from
    /// the 'outputsToRetainBackwardStateFor' outputs of the Function to any of the inputs of the Function, in a
    /// subsequent Backward call. Note that the returned BackPropState instance also stores a reference to the supplied
    /// 'inputs' Values and generated 'outputs' Values and the user is responsible for ensuring that the contents of the
    /// inputs and outputs are unchanged until after any uses of the BackPropState instance for backpropagating
    /// gradients through this Function. User defined Functions that derive from the Function type must implement this
    /// method.
    ///
...
public:
    ...
    ///
    CNTK_API virtual size_t CurrentVersion() const override
    {
        NOT_IMPLEMENTED;
    }
    ...
    ///
    /// Creates a clone of this Function instance, using the specified 'inputs' that are inputs of the clone to be
    /// constructed.
    ///
    CNTK_API virtual FunctionPtr Clone(const std::vector<Variable>& /*clonedInputs*/)
    {
        NOT_IMPLEMENTED;
    }
...
public:
    ...
    CNTK_API void Gradients(const std::unordered_map<Variable, ValuePtr>& arguments, Variable& gradientRoot,
                            std::unordered_map<Variable, ValuePtr>& gradients,
                            std::unordered_map<Variable, ValuePtr>& outputsToEvaluate,
                            const DeviceDescriptor& computeDevice = DeviceDescriptor::UseDefaultDevice());
    ...
    ///
    /// Performs forward computation, i.e. evaluation, on the computaion graph using provided 'input' and stores the
    /// results in the 'outputs' map. It is same as Forward, but without storing and returning information needed for
    /// backpropagation.
    ///
    ...
    ///
    /// Clones 'this' Function. The parameters of the Function are either cloned, shared or frozen as specified by the
    /// parameterCloneMethod argument and any variable replacements requested are applied in the cloned Function
    /// instance.
    ///
    CNTK_API FunctionPtr Clone(ParameterCloningMethod parameterCloneMethod = ParameterCloningMethod::Clone,
                               const std::unordered_map<Variable, Variable>& replacements = {}) const;

    ///
    /// Clones 'this' Function with flattening(removing all block functions). Parameters as above.
    ///
    CNTK_API FunctionPtr
    CloneFlattened(ParameterCloningMethod parameterCloneMethod = ParameterCloningMethod::Share) const;
    ...
    ///
    CNTK_API static FunctionPtr
    Deserialize(const Dictionary& dictionary,
                const ::CNTK::DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice());
...
public:
    ...
    Variable Output() const
    {
        ...
        if (outputs.size() > 1)
            RuntimeError(
                "A Function instance '%S' with more than one output cannot be implicitly converted to a Variable.",
                AsString().c_str());

        if (outputs.empty())
            RuntimeError("A Function instance '%S' with no output cannot be implicitly converted to a Variable.",
                         AsString().c_str());
    ...
    ///
    /// Returns a set comprising of all input variables of 'this' Function's variables that are not of kind 'Parameter'
    /// or 'Constant'.
    ///
    std::vector<Variable> Arguments(bool rowMajor = false) const
    {
        return FilteredInputs<Variable>([](const Variable& var) { return IsArgument(var); }, rowMajor);
    }
    ...
    {
        return FilteredInputs<Parameter>([](const Variable& var) { return var.IsParameter(); });
    }
    ...
    {
        return FilteredInputs<Constant>([](const Variable& var) { return var.IsConstant(); });
    }
    ...
    {
        return FilteredInputs<Variable>([](const Variable& var) { return var.IsPlaceholder(); });
    }

    ///
    ///  Recursively traverses the Function graph underlying 'this' Function invoking the provided functor for all
    ///  visited nodes in the graph. A wrapper for PreorderTraverseFunctions.
    ///
    ...
    /// If nestedSearchInsideBlockFunction is true, all functions inside block functions are also searched for the given
    /// name.
    ///
    FunctionPtr FindByName(const std::wstring& name, bool nestedSearchInsideBlockFunction = false)
    {
        ...
        PreorderTraverseFunctions(RootFunction(),
                                  [&foundFunction, &name, this](const FunctionPtr& function) {
                                      if (name.compare(function->Name()) == 0)
                                      {
                                          if (foundFunction != nullptr)
                                              RuntimeError("FindByName: Multiple functions with the name '%S' are "
                                                           "found in the Function graph underlying 'this' Function.",
                                                           name.c_str());
                                          else
    ...
    std::vector<FunctionPtr> FindAllWithName(const std::wstring& name, bool nestedSearchInsideBlockFunction = false)
    {
        ...
        PreorderTraverseFunctions(RootFunction(),
                                  [&foundFunctions, &name](const FunctionPtr& function) {
                                      if (name.compare(function->Name()) == 0)
...
public:
    ///
    /// Registers a native user-defined Function that can be subsequently instantiated using the
    /// Function::NativeUserFunction method.
    ///
    // TODO: Do we need an Unregister to unload the module?
    CNTK_API static void RegisterNativeUserFunction(const std::wstring& uniqueOpId, const std::wstring& moduleName,
                                                    const std::wstring& factoryMethodName);

    ///
    /// Create an instance of a user-defined Function type registered using Function::RegisterNativeUserFunction method.
    ///
    CNTK_API static FunctionPtr NativeUserFunction(const std::wstring& opId, const std::vector<Variable>& operands,
                                                   const Dictionary& functionConfig,
                                                   const std::wstring& userFunctionInstanceName = L"");

    ///
    /// Register a callback function to be invoked when deserializing a user-defined Function with the corresponding op
    /// name. When loading a model, CNTK will try to automatically reconstruct user-defined Functions (for native
    /// functions, CNTK will invoke the same factory method, the Function op name was registered with). This method
    /// allows to override default user-defined Function deserialization behavior by specifying an op name and the
    /// corresponding callback that should be invoked to inflate the Function object.
    ///
    CNTK_API static void RegisterUDFDeserializeCallback(const std::wstring& uniqueOpName,
                                                        const UDFDeserializeCallback& deserializer);
...
protected:
    ...
    ///
    /// Protected constructors for derived user-defined 'Function' types to specify the actual input and output
    /// variables for the (primitive) Function instance.
    ///
    CNTK_API Function(const std::vector<Variable>& inputs, const Dictionary& functionConfig,
                      const std::wstring& name = L"");
    ...
    static void PreorderTraverseFunctions(const FunctionPtr& rootFunction, const FunctionType& functor,
                                          bool traverseInsideBlockFunction = false)
    {
        std::unordered_set<FunctionPtr> visitedFunctions;
        PreorderTraverseFunctions(rootFunction, visitedFunctions, functor, traverseInsideBlockFunction);
    }

    // Recursively traverses the Function graph underlying the 'rootFunction' invoking the provided functor for all
    // visited nodes in the graph.
    template <typename FunctionType>
    static void PreorderTraverseFunctions(const FunctionPtr& rootFunction,
                                          std::unordered_set<FunctionPtr>& visitedFunctions,
                                          const FunctionType& functor, bool traverseInsideBlockFunction = false)
    {
        visitedFunctions.insert(rootFunction);
        functor(rootFunction);

        if (rootFunction->IsComposite())
            PreorderTraverseFunctions(rootFunction->RootFunction(), visitedFunctions, functor,
                                      traverseInsideBlockFunction);
        else
        {
            if (traverseInsideBlockFunction && rootFunction->IsBlock())
                PreorderTraverseFunctions(rootFunction->BlockRoot(), visitedFunctions, functor,
                                          traverseInsideBlockFunction);
...
private:
    ...
    void ValidateOrUpdateOutputs(std::unordered_map<const Function*, size_t>& visitedFunctions,
                                 bool& recurrentNodeOutputModified, std::vector<Variable>& buffer);
    ...
    static FunctionPtr Clone(const FunctionPtr& clonee, ParameterCloningMethod parameterCloneMethod,
    ...
    static FunctionPtr CloneFunction(const FunctionPtr& clonee, const std::vector<Variable>& clonedInputs);

    static FunctionPtr FlattenFunction(const FunctionPtr& clonee, const std::vector<Variable>& clonedInputs);

    FunctionPtr CloneImpl(ParameterCloningMethod parameterCloneMethod,
                          const std::unordered_map<Variable, Variable>& replacements,
...
private:
    Function(const std::vector<Variable>& inputs, const Dictionary& functionConfig, const FunctionPtr& rootFunction,
             const std::wstring& name, const std::wstring& uid);
...
private:
    ...
    static FunctionPtr DeserializeNativeImpl(const std::vector<Variable>& inputs, const std::wstring& name,
...
///
CNTK_API FunctionPtr ElementAnd(const Variable& leftOperand, const Variable& rightOperand,
                                const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr ElementOr(const Variable& leftOperand, const Variable& rightOperand,
                               const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise XOR logical operation on the input operands.
///
CNTK_API FunctionPtr ElementXor(const Variable& leftOperand, const Variable& rightOperand,
                                const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr TransposeAxes(const Variable& operand, const Axis& axis1, const Axis& axis2,
                                   const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in transpose operation on the specified input operand using the specified
/// permutation
///
CNTK_API FunctionPtr Transpose(const Variable& operand, const std::vector<Axis>& permutation,
                               const std::wstring& name = L"");

///
/// Create an instance of the slice operation on specified tensor input operand
///
CNTK_API FunctionPtr Slice(const Variable& operand, const std::vector<Axis>& axis, const std::vector<int>& beginIndex,
                           const std::vector<int>& endIndex, const std::wstring& name = L"");

///
/// Create an instance of the slice operation on specified tensor input operand
///
CNTK_API FunctionPtr Slice(const Variable& operand, const std::vector<Axis>& axis, const std::vector<int>& beginIndex,
                           const std::vector<int>& endIndex, const std::vector<int>& strides,
                           const std::wstring& name = L"");
...
CNTK_API FunctionPtr Pad(const Variable& operand, PaddingMode mode, const std::vector<size_t>& head,
                         const std::vector<size_t>& foot, double constantValue = 0, const std::wstring& name = L"");

///
/// Create an instance of the random_sample operation on specified sampling weights input vector
///
CNTK_API FunctionPtr RandomSample(const Variable& operand, size_t numSamples, bool allowDuplicates,
                                  unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                  const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr RandomSampleInclusionFrequency(const Variable& operand, size_t numSamples, bool allowDuplicates,
                                                    unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                                    const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr Dropout(const Variable& operand, double dropoutRate,
                             unsigned long seed = SentinelValueForAutoSelectRandomSeed, const std::wstring& name = L"");

///
/// Create an instance of a uniform random operation. This produces random numbers with the specified shape (no dynamic
/// axes), uniformly distributed in [low, high)
///
CNTK_API FunctionPtr UniformRandom(const NDShape& shape, DataType dataType, double low = 0.0, double high = 1.0,
                                   unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                   const std::wstring& name = L"");

///
/// Create an instance of a uniform random operation. This produces random numbers with the shape and dynamic axes
/// specified by the operand, uniformly distributed in [low, high)
///
CNTK_API FunctionPtr UniformRandomLike(const Variable& operand, double low = 0.0, double high = 1.0,
                                       unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                       const std::wstring& name = L"");

///
/// Create an instance of a normal random operation. This produces random numbers with the specified shape (no dynamic
/// axes), normally distributed with the specified mean and standard deviation (scale)
///
CNTK_API FunctionPtr NormalRandom(const NDShape& shape, DataType dataType, double mean = 0.0, double scale = 1.0,
                                  unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                  const std::wstring& name = L"");

///
/// Create an instance of a normal random operation. This produces random numbers with the shape and dynamic axes
/// specified by the operand, normally distributed with the specified mean and standard deviation (scale)
///
CNTK_API FunctionPtr NormalRandomLike(const Variable& operand, double mean = 0.0, double scale = 1.0,
                                      unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                      const std::wstring& name = L"");

///
/// Create an instance of a Gumbel random operation. This produces random numbers with the specified shape (no dynamic
/// axes), distributed according to the Gumbel distribution with the specified location and scale
///
CNTK_API FunctionPtr GumbelRandom(const NDShape& shape, DataType dataType, double loc = 0.0, double scale = 1.0,
                                  unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                  const std::wstring& name = L"");

///
/// Create an instance of a Gumbel random operation. This produces random numbers with the shape and dynamic axes
/// specified by the operand, distributed according to the Gumbel distribution with the specified location and scale
///
CNTK_API FunctionPtr GumbelRandomLike(const Variable& operand, double loc = 0.0, double scale = 1.0,
                                      unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                      const std::wstring& name = L"");

///
/// Create an instance of a Bernoulli random operation. This produces random numbers with the specified shape (no
/// dynamic axes), distributed according to the Bernoulli distribution with the specified success probability
///
CNTK_API FunctionPtr BernoulliRandom(const NDShape& shape, DataType dataType, double mean = 0.5,
                                     unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                     const std::wstring& name = L"");

///
/// Create an instance of a Bernoulli random operation. This produces random numbers with the shape and dynamic axes
/// specified by the operand, distributed according to the Bernoulli distribution with the specified success probability
///
CNTK_API FunctionPtr BernoulliRandomLike(const Variable& operand, double mean = 0.5,
                                         unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                                         const std::wstring& name = L"");

///
/// Create an instance of a flatten operation that reshape the specified tensor into 2D tensor.
/// If input tensor has shape (d_0, d_1, ... d_n) then the output will have shape (d_0 X d_1 ... d_(axis-1), d_axis X
/// d_(axis+1) ... X dn).
///
...
///
CNTK_API FunctionPtr Reshape(const Variable& operand, const NDShape& replacementShape, const Axis& beginAxis,
                             const Axis& endAxis, const std::wstring& name = L"");
...
///
/// Create an instance of a zeros-like operation. This produces zeros with the shape and dynamic axes specified by the
/// operand.
///
CNTK_API FunctionPtr ZerosLike(const Variable& operand, const std::wstring& name = L"");

///
/// Create an instance of a ones-like operation. This produces ones with the shape and dynamic axes specified by the
/// operand.
///
...
/// Create an instance of the CNTK built-in elementwise tensor operation that computes the log of the sum of the
/// exponentials of the specified input operands.
///
CNTK_API FunctionPtr LogAddExp(const Variable& leftOperand, const Variable& rightOperand,
                               const std::wstring& name = L"");

/// Create an instance of the CNTK built-in elementwise tensor operation that computes the leftOperand raised to the
...
///
CNTK_API FunctionPtr ElementTimes(const Variable& leftOperand, const Variable& rightOperand,
                                  const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise division operation on specified tensor input operands.
///
CNTK_API FunctionPtr ElementDivide(const Variable& leftOperand, const Variable& rightOperand,
                                   const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in elementwise equality comparison operation on specified tensor input
/// operands.
///
CNTK_API FunctionPtr Equal(const Variable& leftOperand, const Variable& rightOperand, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise not-equal comparison operation on specified tensor input
/// operands.
///
CNTK_API FunctionPtr NotEqual(const Variable& leftOperand, const Variable& rightOperand,
                              const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise less than comparison operation on specified tensor input
/// operands.
///
CNTK_API FunctionPtr Less(const Variable& leftOperand, const Variable& rightOperand, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise less than or equal to comparison operation on specified tensor
/// input operands.
///
CNTK_API FunctionPtr LessEqual(const Variable& leftOperand, const Variable& rightOperand,
                               const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise greater than comparison operation on specified tensor input
/// operands.
///
CNTK_API FunctionPtr Greater(const Variable& leftOperand, const Variable& rightOperand, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise greater than or equal to comparison operation on specified
/// tensor input operands.
///
CNTK_API FunctionPtr GreaterEqual(const Variable& leftOperand, const Variable& rightOperand,
                                  const std::wstring& name = L"");
...
enum : int
{
    ...
    TimesReduceSequenceAxisWithoutInferredInputRank =
        -2, // reduce sequence axis. Currently only support cases like (m x k) x (k) -> (m) for sequences
};

CNTK_API FunctionPtr Times(const Variable& leftOperand, const Variable& rightOperand, size_t outputRank,
                           int inferInputRankToMap, const std::wstring& name = L"");
...
/// TODO: Document inferInputRankToMap
///
inline FunctionPtr Times(const Variable& leftOperand, const Variable& rightOperand, size_t outputRank,
                         const std::wstring& name = L"")
{
...
/// TODO: Specify the constraints on the shapes of the operands.
///
CNTK_API FunctionPtr TransposeTimes(const Variable& leftOperand, const Variable& rightOperand, size_t outputRank,
                                    const std::wstring& name = L"");
...
/// TODO: Specify the constraints on the shapes of the operands.
///
inline FunctionPtr TransposeTimes(const Variable& leftOperand, const Variable& rightOperand,
                                  const std::wstring& name = L"")
{
...
///
CNTK_API FunctionPtr CosineDistance(const Variable& leftOperand, const Variable& rightOperand,
                                    const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation to compute the cosine distance with negative samplesfor the
/// specified input operands.
///
CNTK_API FunctionPtr CosineDistanceWithNegativeSamples(const Variable& leftOperand, const Variable& rightOperand,
                                                       size_t shiftWindow, size_t numberOfNegativeSamples,
                                                       const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation to compute binary cross-entropy for specified input operands.
///
CNTK_API FunctionPtr BinaryCrossEntropy(const Variable& prediction, const Variable& targets,
                                        const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation to compute weighted binary cross-entropy for specified input
/// operands.
///
CNTK_API FunctionPtr WeightedBinaryCrossEntropy(const Variable& prediction, const Variable& targets,
                                                const Variable& weights, const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in operation to compute cross-entropy with softmax for specified input
/// operands.
///
CNTK_API FunctionPtr CrossEntropyWithSoftmax(const Variable& prediction, const Variable& labels, const Axis& axis,
                                             const std::wstring& name = L"");
...
/// operands.
///
inline FunctionPtr CrossEntropyWithSoftmax(const Variable& prediction, const Variable& labels,
                                           const std::wstring& name = L"")
{
...
///
CNTK_API FunctionPtr EditDistanceError(const Variable& prediction, const Variable& labels, float substitutionPenalty,
                                       float deletionPenalty, float insertionPenalty, bool squashInputs,
...
///
CNTK_API FunctionPtr LatticeSequenceWithSoftmax(
    const Variable& labels, const Variable& prediction, const Variable& scaledLogLikelihood, const Variable& lattice,
    const std::wstring& symbolListPath, const std::wstring& phonePath, const std::wstring& stateListPath,
    const std::wstring& transitionProbabilityPath, const std::wstring& configFilePath, float smoothingWeight,
    float frameDropThreshold, bool doReferenceAlign, bool gammarUsesMBR, float gammarAMF, float gammarLMF,
    float gammarBMMIFactor, float gammarWordPenalty, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation for computing the forwardbackward for specified operands.
///
CNTK_API FunctionPtr ForwardBackward(const Variable& graph, const Variable& features, size_t blankTokenId,
                                     int delayConstraint, const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in operation for computing the classification prediction error for specified
/// operands.
///
CNTK_API FunctionPtr ClassificationError(const Variable& prediction, const Variable& labels, size_t topN,
                                         const Axis& axis, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation for computing the classification prediction error for specified
/// operands.
///
inline FunctionPtr ClassificationError(const Variable& prediction, const Variable& labels, size_t topN,
                                       const std::wstring& name = L"")
{
...
/// operands.
///
inline FunctionPtr ClassificationError(const Variable& prediction, const Variable& labels, const Axis& axis,
                                       const std::wstring& name = L"")
{
...
/// operands.
///
inline FunctionPtr ClassificationError(const Variable& prediction, const Variable& labels,
                                       const std::wstring& name = L"")
{
...
///
CNTK_API FunctionPtr NCELoss(const Variable& weights, const Variable& biases, const Variable& inputs,
                             const Variable& labels, const Constant& noiseWeights, size_t numSamples,
                             bool allowDuplicates = true, unsigned long seed = SentinelValueForAutoSelectRandomSeed,
                             const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr LambdaRank(const Variable& prediction, const Variable& gains, const Variable& groupId,
                                const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation for evaluating the NDCG at 1 metric
///
CNTK_API FunctionPtr NDCGAt1(const Variable& prediction, const Variable& gains, const Variable& groupId,
                             const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation for getting the past value along the lone dynamic axis of the
/// specified operand. Throws an exception of the operand has more than one dynamic axis.
///
CNTK_API FunctionPtr PastValue(const Variable& operand, const Variable& initialState, size_t offset = 1,
                               const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operation for getting the past value along the lone dynamic axis of the
/// specified operand. This overload uses an initial state value of 0. Throws an exception of the operand has more than
/// one dynamic axis.
///
...
///
/// Create an instance of the CNTK built-in operation for getting the future value along the lone dynamic axis of the
/// specified operand. Throws an exception of the operand has more than one dynamic axis.
///
CNTK_API FunctionPtr FutureValue(const Variable& operand, const Variable& initialState, size_t offset = 1,
                                 const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK build-in operation to get the one_hot tensor on specified input along the specified
/// axis
///
CNTK_API FunctionPtr OneHotOp(const Variable& operand, size_t numClass, bool outputSparse, Axis& axis,
                              const std::wstring& name = L"");
...
/// Create an instance of the CNTK build-in operation to get a tensor that is gathered from reference tensor by indices.
///
CNTK_API FunctionPtr GatherOp(const Variable& indices, const Variable& reference, const Axis& axis,
                              const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in sum reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceSum(const Variable& operand, const Axis& axis, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in LogSum reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceLogSum(const Variable& operand, const Axis& axis, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in Mean reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceMean(const Variable& operand, const Axis& axis, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in Max reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceMax(const Variable& operand, const Axis& axis, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in Min reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceMin(const Variable& operand, const Axis& axis, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in Prod reduction operation on specified tensor input operand along the
/// specified axis
///
...
/// specified axis
///
CNTK_API FunctionPtr ReduceLogSum(const Variable& operand, const std::vector<Axis>& axis,
                                  const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in L1 norm reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceL1(const Variable& operand, const std::vector<Axis>& axes, bool keepDims = true,
                              const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in L2 norm reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceL2(const Variable& operand, const std::vector<Axis>& axes, bool keepDims = true,
                              const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in square sum reduction operation on specified tensor input operand along the
/// specified axis
///
CNTK_API FunctionPtr ReduceSumSquare(const Variable& operand, const std::vector<Axis>& axes, bool keepDims = true,
                                     const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in image scaler operation on specified image tensor input operand.
///
CNTK_API FunctionPtr ImageScaler(const Variable& operand, float scaler, std::vector<float> biases,
                                 const std::wstring& name);

///
/// Per dimension mean-variance normalization of the specified input operand.
///
CNTK_API FunctionPtr PerDimMeanVarianceNormalize(const Variable& operand, const Variable& mean,
                                                 const Variable& invStdDev, const std::wstring& name = L"");

///
/// Per dimension mean-variance normalization of the specified input operand.
///
inline FunctionPtr PerDimMeanVarianceNormalize(const Variable& operand, const NDArrayViewPtr& mean,
                                               const NDArrayViewPtr& invStdDev, const std::wstring& name = L"")
{
...
///
CNTK_API FunctionPtr MeanVarianceNormalization(const Variable& operand, double epsilon,
                                               const bool useStatsAcrossChannels = false,
                                               const bool doVarianceScaling = true, const std::wstring& name = L"");

inline FunctionPtr MeanVarianceNormalization(const Variable& operand, const bool useStatsAcrossChannels = false,
                                             const bool doVarianceScaling = true, const std::wstring& name = L"")
{
...
/// Convolution
///
CNTK_API FunctionPtr Convolution(const Variable& convolutionMap, const Variable& operand, const NDShape& strides = {1},
                                 const std::vector<bool>& sharing = {true},
                                 const std::vector<bool>& autoPadding = {true}, const NDShape& dilation = {1},
                                 size_t reductionRank = 1, size_t groups = 1, size_t maxTempMemSizeInSamples = 0,
                                 const std::wstring& name = L"");

///
/// Convolution transpose
///
CNTK_API FunctionPtr ConvolutionTranspose(const Variable& convolutionMap, const Variable& operand,
                                          const NDShape& strides = {1}, const std::vector<bool>& sharing = {true},
                                          const std::vector<bool>& autoPadding = {true},
                                          const NDShape& outputShape = {0}, const NDShape& dilation = {1},
                                          size_t reductionRank = 1, size_t maxTempMemSizeInSamples = 0,
                                          const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in ROI pooling operation on specified tensor input operands with the specified
/// output shape
///
CNTK_API FunctionPtr ROIPooling(const Variable& operand, const Variable& rois, PoolingType poolingType,
                                const NDShape& roiOutputShape, double spatialScale,
                                const std::wstring& name /* = L""*/);

///
/// TODO:
///
CNTK_API FunctionPtr Pooling(const Variable& operand, PoolingType poolingType, const NDShape& poolingWindowShape,
                             const NDShape& strides = {1}, const std::vector<bool>& autoPadding = {false},
                             const bool ceilOutDim = false, const bool includePad = false,
                             const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in Unpooling operation on specified tensor input operands with the specified
/// type and shape
///
CNTK_API FunctionPtr Unpooling(const Variable& operand, const Variable& poolingInput, PoolingType UnpoolingType,
                               const NDShape& UnpoolingWindowShape, const NDShape& strides = {1},
                               const std::vector<bool>& autoPadding = {false}, const std::wstring& name = L"");

///
/// TODO:
///
CNTK_API FunctionPtr BatchNormalization(const Variable& operand, const Variable& scale, const Variable& bias,
                                        const Variable& runningMean, const Variable& runningInvStd,
                                        const Variable& runningCount, bool spatial,
                                        double normalizationTimeConstant = 0, double blendTimeConstant = 0,
                                        double epsilon = 0.00001, bool useCuDNNEngine = true,
                                        bool disableRegularization = false, const std::wstring& name = L"");

//
// Local response normalization as described in
// http://papers.nips.cc/paper/4824-imagenet-classification-with-deep-convolutional-neural-networks
//
CNTK_API FunctionPtr LocalResponseNormalization(const Variable& operand, size_t depthRadius, double bias, double alpha,
                                                double beta, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in OptimizedRNNStack operation on specified input operands
///
CNTK_API FunctionPtr OptimizedRNNStack(const Variable& operand, const Variable& weights, size_t hiddenSize,
                                       size_t numLayers, bool bidirectional = false,
                                       const std::wstring& recurrentOp = L"lstm", const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise clip operation on the tensor operand
///
CNTK_API FunctionPtr Clip(const Variable& operand, const Variable& min, const Variable& max,
                          const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise choice operation using a condition tensor for specified tensor
/// operands.
///
CNTK_API FunctionPtr ElementSelect(const Variable& condition, const Variable& thenOperand, const Variable& elseOperand,
                                   const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in splice operation to splice together all the specified tensor operands into a
/// single output tensor
///
CNTK_API FunctionPtr Splice(const std::vector<Variable>& operands, const Axis& axis, const std::wstring& name = L"");

///
/// Create a new Function instance which just combines the outputs of the specified list of 'operands' Functions such
/// that the 'Outputs' of the new 'Function' are union of the 'Outputs' of each of the specified 'operands' Functions.
/// E.g. When creating a classification model, typically the CrossEntropy loss Function and the ClassificationError
/// Function comprise the two roots of the computation graph which can be "Combine"d to create a single Function with 2
/// outputs; viz. CrossEntropy loss and ClassificationError output.
///
...
///
CNTK_API FunctionPtr AsBlock(FunctionPtr&& composite, const std::vector<std::pair<Variable, Variable>>& argumentsMap,
                             const std::wstring& blockOpName, const std::wstring& blockName = L"");
...
///
CNTK_API FunctionPtr AsBlock(FunctionPtr&& composite, const std::vector<std::pair<Variable, Variable>>& argumentsMap,
                             Dictionary&& attributes, const std::wstring& blockOpName, const std::wstring& blockName);

///
/// Creates a new Function instance which output its input as it is and previent any gradient contribution from its
/// output.
///
...
///
/// Create an instance of the CNTK built-in elementwise exponential linear unit operation with the specified input
/// operand.
///
CNTK_API FunctionPtr ELU(const Variable& operand, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise scaled exponential linear unit operation with the specified
/// input operand.
///
CNTK_API FunctionPtr SELU(const Variable& operand, double gamma = 1.0507009873554804934193349852946,
                          double alpha = 1.6732632423543772848170429916717, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in elementwise leaky linear rectifier operation with the specified input
/// operand.
///
...
///
CNTK_API FunctionPtr ToSequence(const Variable& operand, const std::wstring& sequenceAxisNamePrefix,
                                const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr ToSequence(const Variable& operand, const Variable& sequenceLengths,
                                const std::wstring& sequenceAxisNamePrefix, const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operator for converting the specified tensor operand into a sequence
/// This overload allows specifying an additional 'dynamicAxesLike' operand which is used to obtain the lengths of the
/// generated sequences; the dynamic axes of the generated sequence are required to match the dynamic axes of the
/// 'dynamicAxesLike' operand.
///
CNTK_API FunctionPtr ToSequenceLike(const Variable& operand, const Variable& dynamicAxesLike,
                                    const std::wstring& name = L"");

///
/// Create an instance of the CNTK built-in operator for reconciling the dynamic axes of the specified tensor operands.
/// The output of the returned Function has the sample layout of the left operand and the dynamic axes of the
/// axesAsOperand. It also performs a runtime check to ensure that the  dynamic axes layouts of the 2 operands indeed
/// match.
///
CNTK_API FunctionPtr ReconcileDynamicAxes(const Variable& operand, const Variable& axesAsOperand,
                                          const std::wstring& name = L"");
...
///
/// Create an instance of the CNTK built-in sum reduction operation on specified tensor input operand along the operands
/// lone dynamic sequence axis
///
...
CNTK_API FunctionPtr Gather(const Variable& operand, const Variable& condition,
...
CNTK_API FunctionPtr Scatter(const Variable& operand, const Variable& condition,
                             const std::pair<size_t, int>& newDerivedSequenceAxisScalingAndAdditiveFactor,
                             const std::wstring& name = L"");
...
///
CNTK_API FunctionPtr Unpack(const Variable& operand, double paddingValue, bool supressMaskOutput,
                            const std::wstring& name = L"");
} // namespace Sequence
...
public:
    ...
    ///
    /// A special value that can be used for the minibatchSize to indicate that the reference minibatch size is not
    /// specified.
    ///
    ...
    CNTK_API TrainingParameterSchedule(const std::vector<T>& schedule, size_t epochSize = FullDataSweep,
                                       size_t minibatchSize = IgnoredMinibatchSize);
#endif
    ...
    /// @e minibatchSize is the a minibatch size that each schedule[i] specifies for.
    ///
    CNTK_API TrainingParameterSchedule(const std::vector<std::pair<size_t, T>>& schedule,
                                       size_t epochSize = FullDataSweep, size_t minibatchSize = IgnoredMinibatchSize);
    ...
    bool IsSweepBased() const
    {
        return m_epochSize == FullDataSweep;
    }
    ...
    {
        return this->m_schedule == right.m_schedule && this->m_epochSize == right.m_epochSize &&
               this->m_minibatchSize == right.m_minibatchSize;
    }

    CNTK_API TrainingParameterSchedule<T>& Transform(std::function<T(const T&)> func);

    CNTK_API size_t GetMinibatchSize() const
    {
        return m_minibatchSize;
    }
    CNTK_API void SetMinibatchSize(size_t minibatchSize)
    {
...
template <typename T>
TrainingParameterSchedule<T>
TrainingParameterPerSampleSchedule(const std::vector<T>& schedule,
...
template <typename T>
TrainingParameterSchedule<T>
TrainingParameterPerSampleSchedule(const std::vector<std::pair<size_t, T>>& schedule,
                                   size_t epochSize = TrainingParameterSchedule<T>::FullDataSweep)
{
    return TrainingParameterSchedule<T>(schedule, epochSize, 1);
}
...
/// Construct MomentumAsTimeCosntantSchedule. *Will be deprecated*.
CNTK_API MomentumSchedule MomentumAsTimeConstantSchedule(const std::vector<double>& schedule,
                                                         size_t epoch_size = MomentumSchedule::FullDataSweep);
/// Construct MomentumAsTimeCosntantSchedule. *Will be deprecated*.
CNTK_API MomentumSchedule MomentumAsTimeConstantSchedule(const std::vector<std::pair<size_t, double>>& schedule,
                                                         size_t epoch_size = MomentumSchedule::FullDataSweep);
...
public:
    ...
    //
    virtual bool Update(std::unordered_map<Parameter, NDArrayViewPtr>& gradientValues, size_t trainingSampleCount,
                        bool sweepEnd) = 0;

    ///
    /// Returns the set of parameters associated with this learner.
    ///
    virtual const std::vector<Parameter>& Parameters() const
    {
        return m_parameters;
    }
    ...
    CNTK_API Dictionary& GetOptions()
    {
        return m_additionalOptions.dictOptions;
    }
    CNTK_API const Dictionary& GetOptions() const
    {
        return m_additionalOptions.dictOptions;
    }

    /// In the litature, usually the learner hyper-parameters, such as the learning rates and other hyper-parameters
    /// (such as those in momentum SGD or ADAM), are chosen for the specified minibatch size. However, for efficient
    /// implementation and for distributed training, CNTK can vary the actual minibatch sizes for better computational
    /// efficiency. Therefore CNTK allows users to set the reference minibatch size. CNTK will try its best to adjust
    /// the learning hyper-parameters internally to match the behavior of the learning parameters with the specified
    /// specified minibatch size while the actual minibatch size can vary for better computational performance. If
    /// minibatchSize is set to 0, CNTK will apply the hyper-parameters over the whole minibatch as it is without any
    /// underlying scaling. Note the underlying TrainingParameterSchedule's reference minibatch size setting can over
    /// this reference minibatch size setting and be specialized to its own reference minibatch size. However, this is
    /// only suggested for advanced users.
    CNTK_API void SetMinibatchSize(std::size_t minibatchSize)
    {
        GetOptions().Add(MinibatchSizeKey, minibatchSize);
    }
    CNTK_API std::size_t GetMinibatchSize() const
    {
        return GetOptions().GetOrElse(MinibatchSizeKey, IgnoredMinibatchSize);
    }

    CNTK_API void SetLearningRateSchedule(const LearningRateSchedule& learningRateSchedule)
    {
        m_learningRateSchedule = learningRateSchedule;
    }
    CNTK_API const LearningRateSchedule& GetLearningRateSchedule() const
    {
        return m_learningRateSchedule;
    }

    /// Return whether the learning schedule indicates a literature compatible mode to use mean gradient and potentially
    /// other adjustment of the parameters if necessary.
    template <typename T>
...
protected:
    ...
    Learner(const std::vector<Parameter>& parameters, const LearningRateSchedule& learningRateSchedule,
            const AdditionalLearningOptions& additionalOptions = AdditionalLearningOptions())
...
CNTK_API LearnerPtr MomentumSGDLearner(const std::vector<Parameter>& parameters,
                                       const LearningRateSchedule& learningRateSchedule,
                                       const MomentumSchedule& momentumSchedule, bool unitGain = DefaultUnitGainValue(),
                                       AdditionalLearningOptions additionalOptions = AdditionalLearningOptions());
...
CNTK_API LearnerPtr AdaGradLearner(const std::vector<Parameter>& parameters,
                                   const LearningRateSchedule& learningRateSchedule, bool needAveMultiplier = true,
                                   AdditionalLearningOptions additionalOptions = AdditionalLearningOptions());
...
CNTK_API LearnerPtr RMSPropLearner(const std::vector<Parameter>& parameters,
                                   const LearningRateSchedule& learningRateSchedule, double gamma, double inc,
                                   double dec, double max, double min, bool needAveMultiplier = true,
                                   AdditionalLearningOptions additionalOptions = AdditionalLearningOptions());
...
CNTK_API LearnerPtr AdaDeltaLearner(const std::vector<Parameter>& parameters,
                                    const LearningRateSchedule& learningRateSchedule, double rho = 0.95,
                                    double epsilon = 1e-8,
                                    AdditionalLearningOptions additionalOptions = AdditionalLearningOptions());
...
///
/// Create an instance of a learner by specifying the parameters , gradients and update function. Return a CNTK
/// FunctionPtr.
///
CNTK_API LearnerPtr UniversalLearner(const std::vector<Parameter>& parameters, const std::vector<Variable>& gradients,
                                     FunctionPtr updateFunc);
...
public:
    ...
    bool Update(std::unordered_map<Parameter, NDArrayViewPtr>& gradientValues, size_t minibatchSampleCount,
                bool sweepEnd) override
    {
    ...
    // learning has stopped for all of the parameters associated with this learner
    //
    CNTK_API virtual bool Update(std::unordered_map<Parameter, NDArrayViewPtr>& gradientValues,
                                 MinibatchInfo& minibatch) = 0;

    //
    // In distributed mode all built-in minibatch sources return a minibatch decimated by the number of workers.
    // Some distributed methods (i.e. BlockMomentum) require each worker to run with original/not decimated minibatch
    // size. This method is used by the training session to adapt minibatch size before asking the minibatch source for
    // data. The function returns the scale factor for the minibatch size.
    //
...
protected:
    DistributedLearner(DistributedCommunicatorPtr communicator, LearnerPtr learner, size_t distributeAfterSamples)
        : Learner(learner ? learner->Parameters() : std::vector<Parameter>(), LearningRateSchedule(0)),
          m_learner(learner),
...
CNTK_API DistributedLearnerPtr CreateDataParallelDistributedLearner(DistributedCommunicatorPtr communicator,
                                                                    LearnerPtr learner, size_t distributeAfterSamples,
                                                                    bool useAsyncBufferedParameterUpdate = false);

CNTK_API DistributedLearnerPtr CreateQuantizedDataParallelDistributedLearner(
    QuantizedDistributedCommunicatorPtr communicator, LearnerPtr learner, size_t distributeAfterSamples,
    bool useAsyncBufferedParameterUpdate = false);

CNTK_API DistributedLearnerPtr CreateBlockMomentumDistributedLearner(
    DistributedCommunicatorPtr communicator, LearnerPtr learner, size_t distributeAfterSamples, size_t blockSize,
    double blockMomentumAsTimeConstant, bool useNestrovMomentum = true, bool resetSGDMomentumAfterAggregation = true,
    double blockLearningRate = 1.0);

CNTK_API DistributedLearnerPtr CreateBlockMomentumDistributedLearner(DistributedCommunicatorPtr communicator,
                                                                     LearnerPtr learner, size_t distributeAfterSamples,
                                                                     size_t blockSize, bool useNestrovMomentum = true,
...
public:
    ///
    /// Tests the model on the specified batch of samples using the evaluation Function specified during construction of
    /// the Evaluator Returns the average evaluation criterion value per sample for the tested minibatch of samples
    ///
    CNTK_API double TestMinibatch(const std::unordered_map<Variable, MinibatchData>& arguments,
                                  const DeviceDescriptor& computeDevice = DeviceDescriptor::UseDefaultDevice(),
    ...
    ///
    CNTK_API double TestMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments,
                                  const DeviceDescriptor& computeDevice = DeviceDescriptor::UseDefaultDevice(),
                                  bool distributed = false);
    ...
    ///
    FunctionPtr EvaluationFunction() const
    {
...
private:
    ...
    bool TestMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments, std::pair<ValuePtr, size_t>& result,
                       const DeviceDescriptor& computeDevice, bool distributed = false);
    bool TestMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments,
                       std::unordered_map<Variable, ValuePtr>& outputsToFetch, std::pair<ValuePtr, size_t>& result,
                       const DeviceDescriptor& computeDevice, bool distributed = false);

    std::pair<ValuePtr, size_t> TestLocalMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments,
                                                   std::unordered_map<Variable, ValuePtr>& outputsToFetch,
...
protected:
    Evaluator(const FunctionPtr& evaluationFunction, const std::vector<ProgressWriterPtr>& progressWriters = {},
              bool initializeCombined = true);
    ...
    static std::unordered_map<Variable, ValuePtr>
    GetInputs(const std::unordered_map<Variable, MinibatchData>& arguments);
...
///
CNTK_API EvaluatorPtr CreateEvaluator(const FunctionPtr& evaluationFunction,
...
public:
    ...
    ///
    CNTK_API bool TrainMinibatch(const std::unordered_map<Variable, MinibatchData>& arguments,
                                 const DeviceDescriptor& computeDevice = DeviceDescriptor::UseDefaultDevice());

    ///
    /// An overload of the TrainMinibatch above that takes a map of variables and their values (as its first argument).
    ///
    CNTK_API bool TrainMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments, bool isSweepEndInArguments,
                                 const DeviceDescriptor& computeDevice = DeviceDescriptor::UseDefaultDevice());

    ///
    /// Optimize model parameters using the specified 'arguments' minibatch of training samples.
    /// The variables specified in the 'outputsToFetch' map denote the subset of 'this' Function's output variables that
    /// the caller wants to obtain values of. Callers may specify the storage to be used for storing the 'outputs'
    /// Values or pass null in which case the implementation allocates the actual storage for the 'outputs' for which
    /// the ValuePtr mapping was left null by the caller. Returns false if all parameter learners indicate end of
    /// learning (through their Update method's return value).
    ///
    CNTK_API bool TrainMinibatch(const std::unordered_map<Variable, MinibatchData>& arguments,
                                 std::unordered_map<Variable, ValuePtr>& outputsToFetch,
                                 const DeviceDescriptor& computeDevice = DeviceDescriptor::UseDefaultDevice());
    ...
    ///
    /// Restore the model and trainer state from a previously saved model and checkpoint from the specified file
    /// location
    ///
    ...
    ///
    FunctionPtr LossFunction() const
    {
    ...
    ///
    size_t PreviousMinibatchSampleCount() const
    {
        return m_prevMinibatchNumSamples;
    }
...
private:
    ...
    Trainer(const FunctionPtr& model, const FunctionPtr& lossFunction, const std::vector<LearnerPtr>& parameterLearners,
            const std::vector<ProgressWriterPtr>& progressWriters = {});
    Trainer(const FunctionPtr& model, const FunctionPtr& lossFunction, const FunctionPtr& evaluationFunction,
            const std::vector<LearnerPtr>& parameterLearners,
            const std::vector<ProgressWriterPtr>& progressWriters = {});

    void ExecuteForwardBackward(const std::unordered_map<Variable, ValuePtr>& arguments,
                                std::unordered_map<Variable, ValuePtr>& outputsToFetch,
                                const DeviceDescriptor& computeDevice,
                                std::unordered_map<Variable, ValuePtr>& parameterGradients);

    bool TrainLocalMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments,
                             std::unordered_map<Variable, ValuePtr>& outputsToFetch, bool sweepEnd,
                             const DeviceDescriptor& computeDevice);
    bool TrainDistributedMinibatch(const std::unordered_map<Variable, ValuePtr>& arguments,
                                   std::unordered_map<Variable, ValuePtr>& outputsToFetch, bool sweepEnd,
                                   const DeviceDescriptor& computeDevice);
...
///
/// Construct a Trainer to train the specified 'model' with the specified 'trainingLoss' Variable as the training
/// criterion and using the specified set of 'parameterLearners' for updating the model's parameters using computed
/// gradients.
///
CNTK_API TrainerPtr CreateTrainer(const FunctionPtr& model, const FunctionPtr& lossFunction,
                                  const std::vector<LearnerPtr>& parameterLearners,
                                  const std::vector<ProgressWriterPtr>& progressWriters = {});

///
/// Construct a Trainer to train the specified 'model' with the specified 'trainingLoss' as the training criterion,
/// the specified 'evaluationFunction' as the criterion for evaluating the trained model's quality, and using the
/// specified set of 'parameterLearners' for updating the model's parameters using computed gradients.
///
CNTK_API TrainerPtr CreateTrainer(const FunctionPtr& model, const FunctionPtr& lossFunction,
...
struct MinibatchData
{
    ...
    std::wstring AsString() const
    {
        ...
        wss << L"MinibatchData(data=" << data->AsString() << L", samples=" << numberOfSamples << L", seqs="
            << numberOfSequences << L")";
        return wss.str();
    }
...
public:
    ...
    /// The minibatch size is specified in terms of #samples and/or #sequences for the primary input stream; value of 0
    /// for #samples/#sequences means unspecified. In case the size is specified in terms of both #sequences and
    /// #samples, the smaller of the 2 is taken. An empty map is returned when the MinibatchSource has no more data to
    /// return.
    ///
    CNTK_API const std::unordered_map<StreamInformation, MinibatchData>&
    GetNextMinibatch(size_t minibatchSizeInSequences, size_t minibatchSizeInSamples,
                     const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice());
    ...
    ///
    CNTK_API virtual const std::unordered_map<StreamInformation, MinibatchData>&
    GetNextMinibatch(size_t minibatchSizeInSequences, size_t minibatchSizeInSamples, size_t numberOfWorkers,
                     size_t workerRank, const DeviceDescriptor& device = DeviceDescriptor::UseDefaultDevice()) = 0;
...
public:
    ...
    ///
    /// Gets the description of the stream that matches the attributes (Shape, DataType and StorageFormat) of the
    /// specified Variable object Throws an exception if there are none or multiple streams matching the Variable's
    /// attributes
    ///
...
{
    StreamConfiguration(const std::wstring& streamName, size_t dim, bool isSparse = false,
                        const std::wstring& streamAlias = L"", bool definesMbSize = false)
        : m_streamName(streamName),
          m_dim(dim),
          m_isSparse(isSparse),
          m_streamAlias(streamAlias),
          m_definesMbSize(definesMbSize)
    {
    }
...
{
    HTKFeatureConfiguration(const std::wstring& streamName, const std::wstring& scp, size_t dim, size_t left,
                            size_t right, bool broadcast, bool definesMbSize = false,
                            size_t maxSequenceLength = SIZE_MAX)
        : m_streamName(streamName),
          m_dim(dim),
          m_scp(scp),
          m_left(left),
...
///
CNTK_API ImageTransform ReaderScale(int width, int height, int channels, const wchar_t* interpolations = L"linear",
...
///
CNTK_API ImageTransform ReaderColor(float brightnessRadius = 0.0f, float contrastRadius = 0.0f,
                                    float saturationRadius = 0.0f);

///
/// Create an ImageDeserializer with the specified options
///
CNTK_API Deserializer ImageDeserializer(const std::wstring& fileName, const std::wstring& labelStreamName,
                                        size_t numLabels, const std::wstring& imageStreamName,
                                        const std::vector<ImageTransform>& transforms = {});

///
/// Create a Base64ImageDeserializer with the specified options
///
CNTK_API Deserializer Base64ImageDeserializer(const std::wstring& fileName, const std::wstring& labelStreamName,
                                              size_t numLabels, const std::wstring& imageStreamName,
                                              const std::vector<ImageTransform>& transforms = {});
...
///
CNTK_API Deserializer CBFDeserializer(const std::wstring& fileName,
                                      const std::vector<StreamConfiguration>& streams = {});
...
///
CNTK_API Deserializer HTKMLFDeserializer(const std::wstring& streamName, const std::wstring& labelMappingFile,
                                         size_t dimension, const std::vector<std::wstring>& mlfFiles,
                                         bool phoneBoundaries = false);
...
///
inline MinibatchSourcePtr
TextFormatMinibatchSource(const std::wstring& dataFilePath, const std::vector<StreamConfiguration>& streamConfigs,
                          size_t epochSize = MinibatchSource::InfinitelyRepeat, bool randomize = true,
...
///
/// Compute the per dimension means and variances for each of the specified streams using data from the specified
/// minibatchSource.
///
CNTK_API void ComputeInputPerDimMeansAndInvStdDevs(
    const MinibatchSourcePtr& minibatchSource,
...
/// by launching multiple threads and performing a compute operation inside, it will result in each of those
/// concurrently executing operations to use the specified number of CPU threads limit.
///
CNTK_API void SetMaxNumCPUThreads(size_t numCPUThreads);

///
/// Returns the current process-wide setting for maximum number of CPU threads to be used by any individual compute
/// operation
///
...
public:
    ...
    CNTK_API virtual DistributedCommunicatorPtr
    SubGroup(const std::unordered_set<DistributedWorkerDescriptor>& subGroupWorkers) const = 0;

    // A collective communication API to concatenate values across each worker of this communicator. The concatenated
    // values are only sent to the specified workers; for all others the returned Values are null
    CNTK_API virtual void Concatenate(const std::vector<ValuePtr>& values, std::vector<ValuePtr>& outputValues,
                                      const std::unordered_set<DistributedWorkerDescriptor>& sendToWorkers) = 0;

    CNTK_API virtual void Concatenate(const std::vector<NDArrayViewPtr>& input, std::vector<NDArrayViewPtr>& output,
                                      const std::unordered_set<DistributedWorkerDescriptor>& sendToWorkers) = 0;

    // Gathers the inputs from a subset of workers on the main worker.
    CNTK_API virtual void Gather(const Dictionary& input, std::vector<DictionaryPtr>& output,
                                 const std::unordered_set<DistributedWorkerDescriptor>& sendToWorkers) = 0;
    ...
    CNTK_API virtual void AggregateInPlace(const std::vector<NDArrayViewPtr>& values,
                                           const std::unordered_set<DistributedWorkerDescriptor>& sendToWorkers) = 0;

    CNTK_API virtual void AllReduceSparseBlockColumn(std::vector<NDArrayViewPtr>&)
    {
        LogicError("This function should not be reached.");
    }

    CNTK_API virtual void Aggregate(const std::vector<NDArrayViewPtr>& values,
...
public:
    ...
    CNTK_API virtual void QuantizedAggregate(const std::vector<NDArrayViewPtr>& inValues,
    ...
    CNTK_API virtual void
    QuantizedAggregateInPlace(std::vector<NDArrayViewPtr>& inValues,
...
///
CNTK_API QuantizedDistributedCommunicatorPtr QuantizedMPICommunicator(bool zeroThresholdFor1Bit,
                                                                      bool useQuantizationForSelfStripe,
                                                                      size_t numQuantizationBits);
...
public:
    ...
    ///
    CNTK_API CheckpointConfig(const std::wstring& checkPointFileName,
                              size_t checkpointFrequency = std::numeric_limits<size_t>::max(),
                              DataUnit checkpointFrequencyUnit = DataUnit::Sample,
                              bool restoreFromCheckpointIfExists = true, bool preserveAllCheckpoints = false);
...
/// Derived classes can redefine different aspects of training, overriding base virtual methods (GetMinibatchSize,
/// OnMinibatchStart, etc.)
///
...
public:
    ...
    ///
    CNTK_API TrainingSession(const TrainerPtr& trainer, const MinibatchSourcePtr& trainingSource,
                             const MinibatchSizeSchedule& minibatchSizeSchedule,
                             const std::unordered_map<Variable, StreamInformation>& inputVarToStream,
                             size_t maxNumTrainingSamples, size_t progressFrequency, DataUnit progressFrequencyUnit,
                             const CheckpointConfig& checkpointing, const CrossValidationConfig& crossValidation,
...
public:
    ...
    ///
    CNTK_API virtual bool OnMinibatchEnd()
    {
        return true;
    };
    ...
    /// If return value is false, the training will be stopped.
    ///
    CNTK_API virtual bool OnCrossValidationEnd(size_t /*validationIndex*/, double /*averageError*/,
                                               size_t /*numberOfSamples*/, size_t /*numberOfMinibatches*/)
    {
        return true;
    }
...
private:
    ...
    // Auxiliary functions.
    void GetNextMinibatch(const MinibatchSourcePtr& source, std::unordered_map<Variable, ValuePtr>& minibatch,
                          const std::unordered_map<Variable, StreamInformation>& inputVarToStream,
                          bool* pIsMinibatchAtSweepEnd, size_t maxMbSize, size_t workerRank, size_t numberOfWorkers,
                          const DeviceDescriptor& computeDevice);
    void GetTrainingMinibatch(std::unordered_map<Variable, ValuePtr>& minibatch, bool* pIsMinibatchAtSweepEnd,
                              size_t maxMbSize, const DeviceDescriptor& computeDevice);
    void GetCrossValidationMinibatch(std::unordered_map<Variable, ValuePtr>& minibatch, bool* pIsMinibatchAtSweepEnd,
                                     size_t maxMbSize, const DeviceDescriptor& computeDevice);
...
/// Creates an instance of the training session class. Parameters match the parameters of the TrainingSession
/// constructor.
///
CNTK_API TrainingSessionPtr CreateTrainingSession(
    const TrainerPtr& trainer, const MinibatchSourcePtr& trainingSource,
    const MinibatchSizeSchedule& minibatchSizeSchedule,
    const std::unordered_map<Variable, StreamInformation>& inputVarToStream, size_t maxNumTrainingSamples,
    size_t progressFrequency, DataUnit progressFrequencyUnit, const CheckpointConfig& checkpointing = {L""},
    const CrossValidationConfig& crossValidation = {nullptr}, const TestConfig& test = {nullptr});
...
///
CNTK_API FunctionPtr Crop(const Variable& nodeInput, const Variable& nodeReferent, size_t offsetX, size_t offsetY,
                          const std::wstring& name = L"");
...
/// ancestorReferent: ancestor of nodeReferent which is treated as equal to ancestorInput for the purpose of computing
/// crop offsets.
///
CNTK_API FunctionPtr Crop(const Variable& nodeInput, const Variable& nodeReferent, const Variable& ancestorInput,
                          const Variable& ancestorReferent, const std::wstring& name = L"");
